...
export default function Header() {
    ...
    /** Parse subject and topic from a raw notification message string */
    ...
    /** Format a raw message into a readable label, e.g. "📌 Physics: Kinematics" */
    const formatNotificationMessage = (message: string) => {
        const parsed = parseSubjectAndTopic(message);
        if (parsed) {
            return `📌 ${parsed.subject}: ${parsed.topic} results are now available.`;
        }
        return message;
    };

    /** Format ISO timestamp to IST locale string, e.g. "04 Jun, 09:30 AM" */
                            ...
                            <div ref={notificationsRef} style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
                                ...
                                {showNotifications && (
                                    <div style={{
                                        ...
                                        boxShadow: '0 8px 24px rgba(0,0,0,0.12)', width: '320px', zIndex: 100,
                                        maxHeight: '420px', overflowY: 'auto',
                                        animation: 'subjectFadeIn 0.2s ease-out forwards',
                                    }}>
                                        <div style={{ 
                                        ...
                                        {notifications.length > 0 ? (
                                            notifications.map(n => (
                                                <div 
                                                    key={n.id} 
                                                    style={{ padding: '0.85rem 1rem', borderBottom: '1px solid #f1f5f9', cursor: 'pointer', transition: 'background 0.15s' }}
                                                    onClick={async () => {
                                                ...
                                                >
                                                    <p style={{ fontSize: '0.875rem', color: '#1e293b', fontWeight: 500, margin: '0 0 0.3rem', lineHeight: 1.45 }}>
